...
#define WHEEL_CIRC .96
#define WHEEL_WIDTH .54
#define WHEEL_CENTER .27
...
{
	double output;			//current angle
	double current_pos;		//current angle position
	double current_vel;		//current velocity - gyro
	double *last_pos;		//pointer to last_pos -- refactor this out
}IMU_POSITION;
...
typedef struct SPEED_PID
{
	...
	float setPoint[20];		// the set-point calculated use 20 because we average
	float setPoint_offset;	//used to change setpoint offset for the angle - if center of gravity changes
	float total_vel[20];
}SPEED_PID;
...
long wheel_L_cpos;	//current position of left wheel
long wheel_R_cpos;	//current position of right wheel
long wheel_L_fpos;	//final position of left wheel
long wheel_R_fpos;	//final position right wheel
float current_orientation;	//value is in RADIANS -- CONVERT TO ANGLE
