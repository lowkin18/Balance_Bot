...
void init_pid_values()
{
    ...
    _PIDA.Pgain = 3800;			// good base value 3800;
    //TAKES IMU ANGLE DATA EULER PITCH
    _PIDA.Igain = 220;			// good base values 220;
    //TAKES IMU ANGLE DATA EULER PITCH += EULER PITCH NOW
    _PIDA.Dgain = 320;			// good base value 315;
    //TAKES IMU GYROSCOPE DATA


	_PIDS.Pgain = 0.00365;		//good base value 0.00365;
	//TAKES ENCODER VELOCITY
    _PIDS.Igain = 0.000200;		//good base value 0.000170;
    //TAKES ENCODER VELOCITY += ENCODER_VELOCITY NOW
    _PIDS.Dgain = 0.040;		//good base value 0.045
    //TAKES ENCODER VELOCITY_FINAL - VELOCITY_PREVIOUS


    PID_CONTROL_DATA._pid_position.Pgain = 0.00430; // good base value 0.00430
    //TAKES SET POINT ENCODER POSITION - VARIABLE ENCODER POSITION
	PID_CONTROL_DATA._pid_position.Igain = 0.000360;  //good base value 0.000360
	//TAKES THE SET_POINT ERROR CUMULATIVE
	PID_CONTROL_DATA._pid_position.Dgain = 0.98;	//good base value 0.96
...
float PID_pos()
{
	...
	float Perror_position =  (ctrl_robot.wheel_L_fpos  - ctrl_robot.wheel_L_cpos[cycle_data[speed_loop].now] +
							  ctrl_robot.wheel_R_fpos - ctrl_robot.wheel_R_cpos[cycle_data[speed_loop].now])/2;
	//Derror is the velocity error you are at I use both left and right and average them
	float Derror = (qei_data.left_wheel.vel[cycle_data[speed_loop].now] + qei_data.left_wheel.vel[cycle_data[speed_loop].back1] +
					...
					qei_data.right_wheel.vel[cycle_data[speed_loop].back2] +qei_data.right_wheel.vel[cycle_data[speed_loop].back3])/8;

	//Ierror is the accumulated Perror Values - this is capped at 400000 need to refactor this as well
	//** REFACTOR **//
	if(PID_CONTROL_DATA._pid_position.Ierror >100000)
	{
		PID_CONTROL_DATA._pid_position.Ierror = 100000;
	}
	if(PID_CONTROL_DATA._pid_position.Ierror< -100000)
	{
		PID_CONTROL_DATA._pid_position.Ierror = -100000;
	...
	if(KPerror > 10000) KPerror = 10000;
	if(KPerror <-10000)	KPerror = -10000;
	if(KDerror > 10000)	KDerror = 10000;
	if(KDerror <-10000)	KDerror = -10000;
	if(KIerror >2000) KIerror = 2000;
	if(KIerror<-2000) KIerror=2000;
	...
	if(ctrl_robot.current_state == 0 || ctrl_robot.current_state == 1)return speed;
	if(ctrl_robot.current_state == 2 || ctrl_robot.current_state == 3)return 50;
	return 0;
}
...
void PID_speed(float speed)
{
	...
	Derror = ((qei_data.left_wheel.accel[cycle_data[speed_loop].now] + qei_data.left_wheel.accel[cycle_data[speed_loop].back1] +
			...
			qei_data.right_wheel.accel[cycle_data[speed_loop].back2]+ qei_data.right_wheel.accel[cycle_data[speed_loop].back3])/8);
	...
	_PIDS.Ierror += Perror;
	if(_PIDS.Ierror > 8000) _PIDS.Ierror = 8000; // THESE NUMBERS PLAY A LARGE ROLE IN THE INTEGRAL PART
	if(_PIDS.Ierror < -8000)_PIDS.Ierror =-8000; // THESE NUMBERS NEED TO BE TUNED FOR DESIRED MAX INTEGRAL
...
*/
	...
	if(KPerror>20)KPerror  =  20;		//CAPPING THE PROPORTIONAL ERROR
	if(KPerror<-20)KPerror = -20;		//CAPPING THE PROP
	if(KDerror>20)KDerror =  20;		//CAPPING DERIVATIVE ERROR
	if(KDerror<-20)KDerror = -20;		//CAPPING DERIVATIVE ERROR
	if(KIerror>7.5)KIerror  =  7.5;		//CAPPING INTEGRAL ERROR
	if(KIerror<-7.5)KIerror= -7.5;		//CAPPING INTEGRAL ERROR
	...
	if(_PIDS.setPoint > 20.0)_PIDS.setPoint = 20.0; //CAPPING SETPOINT TO 20 Degrees
	if(_PIDS.setPoint <-20.0)_PIDS.setPoint =-20.0;
...
void PID_angle(float pos)
{
	...
	errorP = _PIDA.setPoint-imu_data[angle_loop].euler_values.p-.15;
	errorD = 0;
	_PIDA.Ierror = errorP;
	}
...
void left_wheel_pivot()
{
	...
	qei_data.right_wheel.pwm_out *= 1.7;
	if(qei_data.right_wheel.pwm_out<0)
	{
	qei_data.right_wheel.wheel_dir = -1;
	qei_data.right_wheel.pwm_out = fabs(qei_data.right_wheel.pwm_out);
...
void right_wheel_pivot()
{
		...
		qei_data.left_wheel.pwm_out *= 1.7;
		if(qei_data.left_wheel.pwm_out<0)
		{
		qei_data.left_wheel.wheel_dir = -1;
		qei_data.left_wheel.pwm_out = fabs(qei_data.left_wheel.pwm_out);
